...
Deliver in this order: shared contracts and configuration; artifact templates and persistence; interview and design integration; review/change handling; deterministic structural validation and scoring; evaluator/report publishing; brownfield/resume scenarios; compatibility and documentation. Each task must name its dependencies, affected paths and observable acceptance criteria. Tests cover happy paths and failed gates, including missing evidence, no response, invalid weights, stale reports, concurrent updates and interrupted sessions.
